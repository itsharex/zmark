  ...
  return (
    <div className="flex items-center gap-1">
      {keys.map((k) => (
        <kbd
          key={k}
          className="flex h-5 min-w-5 w-fit items-center justify-center rounded-md border border-b-2 bg-muted font-sans text-xs! font-medium text-muted-foreground shadow-sm"
        >
          {k}
        </kbd>
      ))}
